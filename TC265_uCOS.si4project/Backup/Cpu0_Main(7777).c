...
//    CPU_Init();
//
//    OS_CPU_SysTickInit();                                       /* Init uC/OS periodic time src (SysTick).              */
//
//
//
...
void AppTask1 ()
{
    ...
    while (TRUE) {
        OSTimeDly(1000u * OSCfg_TickRate_Hz,               /* Wait until statistic task is ready                     */
                  OS_OPT_TIME_DLY,
                  &err);
        CAN_SendSingle(&canMsgObj1, 0x102, 0x0000, 0x0000);
    }
}
...
void AppTask2 ()
{
    ...
    while (TRUE) {
        OSTimeDly(2u * OSCfg_TickRate_Hz,               /* Wait until statistic task is ready                     */
                  OS_OPT_TIME_DLY,
                  &err);

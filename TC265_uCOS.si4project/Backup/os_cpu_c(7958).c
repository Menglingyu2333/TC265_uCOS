...
//    IfxCpu_Id CpuID = IfxCpu_getCoreId();
    ...
    if(OSPrioCur[CpuID] == 10 && OSPrioHighRdy[CpuID] != 10)
        OSTaskSwHook();

    if(OSPrioHighRdy[CpuID] == 10 && CpuID == 0)
        OSTaskSwHook();
    ...
    ptUpperCTX->_PCXI = *((typeTaskPCXI*)(OSTCBCurPtr[CpuID]->StkPtr));

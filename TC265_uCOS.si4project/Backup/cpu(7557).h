...
#define  OS_ICR_ADDR            0xFE2C
#define  OS_ICR_IE              0x00000100
#define  CPU_INT_DIS()         do { cpu_sr = (__mfcr(OS_ICR_ADDR) & OS_ICR_IE); __asm ("disable"); } while (0) /* Save    CPU status word & disable interrupts.*/
#define  CPU_INT_EN()          do { if ((cpu_sr) != 0) { __asm ("enable"); } } while (0) /* Restore CPU status word.                     */

...
CPU_DATA   OSPrioTbl[CPU_CORE_NUM][OS_PRIO_TBL_SIZE];       /* Declare the array local to this file to allow for  ... */
...
void  OS_PrioInit (void)
{
    ...
    for(j = 0u; j < CPU_CORE_NUM; j++){
                                                            /* Clear the bitmap table ... no task is ready            */
        for (i = 0u; i < OS_PRIO_TBL_SIZE; i++) {
             OSPrioTbl[j][i] = (CPU_DATA)0;
        }
    }
}
#ifdef  SMP_EN
...
void  OS_PrioGetHighest (void)
{
    ...
    for(i = 0; i < CPU_CORE_NUM; i++){
        ...
        while (*p_tbl == (CPU_DATA)0) {                         /* Search the bitmap table for the highest priority       */
            OSPrioHighRdy[i] += DEF_INT_CPU_NBR_BITS;           /* Compute the step of each CPU_DATA entry                */
            p_tbl++;
        }
        OSPrioHighRdy[i] += (OS_PRIO)CPU_CntLeadZeros(*p_tbl);             /* Find the position of the first bit set at the entry    */
        OSTCBHighRdyPtr[i]  = OSRdyList[OSPrioHighRdy[i]].HeadPtr;
    }
}
/*
************************************************************************************************************************
...
void  OS_PrioInsert (OS_TCB        *p_tcb)
{
    CPU_DATA  bit;
    CPU_DATA  bit_nbr;
    OS_PRIO   ix;
    ...
    if(p_tcb == 0){
        ix             = 0 / DEF_INT_CPU_NBR_BITS;
        bit_nbr        = (CPU_DATA)0 & (DEF_INT_CPU_NBR_BITS - 1u);
        bit            = 1u;
        bit          <<= (DEF_INT_CPU_NBR_BITS - 1u) - bit_nbr;
        OSPrioTbl[0][ix] |= bit;
    }else{
        volatile OS_TCB_CTX_EXT *ext;                     /* pointer to TCB extension                 */
        ext = (volatile OS_TCB_CTX_EXT*)p_tcb->ExtPtr;

        ix             = p_tcb->Prio / DEF_INT_CPU_NBR_BITS;
        bit_nbr        = (CPU_DATA)p_tcb->Prio & (DEF_INT_CPU_NBR_BITS - 1u);
        bit            = 1u;
        bit          <<= (DEF_INT_CPU_NBR_BITS - 1u) - bit_nbr;
        OSPrioTbl[ext->CoreID][ix] |= bit;
    }
}
...
void  OS_PrioRemove (OS_TCB        *p_tcb)
{
    CPU_DATA  bit;
    CPU_DATA  bit_nbr;
    OS_PRIO   ix;

...
void OSCtxSw(void)
{
    ...
    IfxCpu_Id CupID = IfxCpu_getCoreId();
    ...
    OSTCBCurPtr[CupID] = OSTCBHighRdyPtr[CupID];      /* new highest prio task is current task    */
    OSPrioCur[CupID] = OSPrioHighRdy[CupID];          /* new highest prio is now current prio     */

    ptUpperCTX->_PCXI = *((typeTaskPCXI*)(OSTCBCurPtr[CupID]->StkPtr));

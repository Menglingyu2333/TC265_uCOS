...
extern IfxCpu_mutexLock  MultiCoreMutex;

#define  CPU_INT_DIS() do { cpu_sr = disableInterrupts(); \
                            while(IfxCpu_acquireMutex(&MultiCoreMutex)){\
                                wait(TimeConst_1us);}} while (0) /* Save    CPU status word & disable interrupts.*/
#define  CPU_INT_EN()  do { restoreInterrupts((boolean)cpu_sr);\
                            IfxCpu_releaseMutex(&MultiCoreMutex);} while (0) /* Restore CPU status word.                     */
